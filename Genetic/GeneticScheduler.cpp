...
int GeneticScheduler::compare_citizen(citizen a, citizen b)
{
    if (a.fitness == b.fitness) return EQUAL;
    if (a.fitness < b.fitness)  return SMALLER;
    if (a.fitness > b.fitness)  return BIGGER;
}
...
bool GeneticScheduler::create_population(vector<citizen> &pop) {
    ...
    for (int i = 0; i < POP_SIZE; i++) {
        ...
        int rn = random_processor_number();
        for (int j = 0; j < nJobs; j++) {
